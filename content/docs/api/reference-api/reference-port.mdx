...
```bash
curl -X POST "http://localhost:3000/api/trpc/port.create" \
  -d '{
  "publishedPort": 0,
  "targetPort": 0,
  "protocol": "tcp",
...
```js
fetch("http://localhost:3000/api/trpc/port.create", {
  method: "POST"
});
```
...
```bash
curl -X GET "http://localhost:3000/api/trpc/port.one?portId=string"
```
...
```js
fetch("http://localhost:3000/api/trpc/port.one?portId=string", {
  method: "GET"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/port.delete" \
  -d '{
  "portId": "string"
}'
```
...
```js
fetch("http://localhost:3000/api/trpc/port.delete", {
  method: "POST"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/port.update" \
  -d '{
  "portId": "string",
  "publishedPort": 0,
  "targetPort": 0,
...
```js
fetch("http://localhost:3000/api/trpc/port.update", {
  method: "POST"
});
```
