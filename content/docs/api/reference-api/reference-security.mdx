...
```bash
curl -X POST "http://localhost:3000/api/trpc/security.create" \
  -d '{
...
```js
fetch("http://localhost:3000/api/trpc/security.create", {
  method: "POST"
});
```
...
```bash
curl -X GET "http://localhost:3000/api/trpc/security.one?securityId=string"
```
...
```js
fetch("http://localhost:3000/api/trpc/security.one?securityId=string", {
  method: "GET"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/security.delete" \
  -d '{
  "securityId": "string"
}'
```
...
```js
fetch("http://localhost:3000/api/trpc/security.delete", {
  method: "POST"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/security.update" \
  -d '{
  "securityId": "string",
  "username": "string",
  "password": "string"
...
```js
fetch("http://localhost:3000/api/trpc/security.update", {
  method: "POST"
});
```
