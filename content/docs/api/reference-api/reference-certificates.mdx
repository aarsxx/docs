...
```bash
curl -X POST "http://localhost:3000/api/trpc/certificates.create" \
  -d '{
...
```js
fetch("http://localhost:3000/api/trpc/certificates.create", {
  method: "POST"
});
```
...
```bash
curl -X GET "http://localhost:3000/api/trpc/certificates.one?certificateId=string"
```
...
```js
fetch("http://localhost:3000/api/trpc/certificates.one?certificateId=string", {
  method: "GET"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/certificates.remove" \
  -d '{
  "certificateId": "string"
}'
```
...
```js
fetch("http://localhost:3000/api/trpc/certificates.remove", {
  method: "POST"
});
```
...
```bash
curl -X GET "http://localhost:3000/api/trpc/certificates.all"
```
...
```js
fetch("http://localhost:3000/api/trpc/certificates.all", {
  method: "GET"
});
```
