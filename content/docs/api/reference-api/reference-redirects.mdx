...
```bash
curl -X POST "http://localhost:3000/api/trpc/redirects.create" \
  -d '{
  "regex": "string",
  "replacement": "string",
  "permanent": true,
...
```js
fetch("http://localhost:3000/api/trpc/redirects.create", {
  method: "POST"
});
```
...
```bash
curl -X GET "http://localhost:3000/api/trpc/redirects.one?redirectId=string"
```
...
```js
fetch("http://localhost:3000/api/trpc/redirects.one?redirectId=string", {
  method: "GET"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/redirects.delete" \
  -d '{
  "redirectId": "string"
}'
```
...
```js
fetch("http://localhost:3000/api/trpc/redirects.delete", {
  method: "POST"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/redirects.update" \
  -d '{
  "redirectId": "string",
  "regex": "string",
  "replacement": "string",
...
```js
fetch("http://localhost:3000/api/trpc/redirects.update", {
  method: "POST"
});
```
