...
```bash
curl -X POST "http://localhost:3000/api/trpc/mounts.create" \
  -d '{
  "type": "bind",
  "hostPath": "string",
  "volumeName": "string",
...
```js
fetch("http://localhost:3000/api/trpc/mounts.create", {
  method: "POST"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/mounts.remove" \
  -d '{
  "mountId": "string"
}'
```
...
```js
fetch("http://localhost:3000/api/trpc/mounts.remove", {
  method: "POST"
});
```
...
```bash
curl -X GET "http://localhost:3000/api/trpc/mounts.one?mountId=string"
```
...
```js
fetch("http://localhost:3000/api/trpc/mounts.one?mountId=string", {
  method: "GET"
});
```
...
```bash
curl -X POST "http://localhost:3000/api/trpc/mounts.update" \
  -d '{
  "mountId": "string",
  "type": "bind",
  "hostPath": "string",
...
```js
fetch("http://localhost:3000/api/trpc/mounts.update", {
  method: "POST"
});
```
